...
CREATE OR REPLACE STAGE BLAZEJ_DB.STAGE.MY_INTERNAL_STAGE
COMMENT = 'Przykladowy internal Stage';
LS @BLAZEJ_DB.STAGE.MY_INTERNAL_STAGE;
...
COPY INTO @BLAZEJ_DB.STAGE.MY_INTERNAL_STAGE FROM
...
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TABLE_WITH_STAGE(
    ID NUMBER,
    MSG TEXT
);

-- Dostęp znak '%' przed nazwą tabeli
LS @BLAZEJ_DB.STAGE.%TABLE_WITH_STAGE;

COPY INTO @BLAZEJ_DB.STAGE.%TABLE_WITH_STAGE FROM
(SELECT 1 AS ID, 'test' AS MSG)
FILE_FORMAT = (TYPE = CSV);
...
CREATE STAGE BLAZEJ_DB.STAGE.QUICKSTART_STAGE
    URL = 's3://sfquickstarts/';
LS @BLAZEJ_DB.STAGE.QUICKSTART_STAGE;
--Obiekt typu Stage umożliwiający interakcje z AWS S3 
CREATE STAGE BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE
    URL = 's3://sfquickstarts/data-engineering-with-snowpark-python/';

--Sprawdzenie zawartośći Stage/ S3 
LIST @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE;
...
SELECT * FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/customer;     

--Obiekt typu FILE FORMAT pozwalajacy na czytanie plików w formacie Parquet z kompresją snappy
-- https://docs.snowflake.com/en/sql-reference/sql/create-file-format
CREATE FILE FORMAT IF NOT EXISTS BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    TYPE = PARQUET
    COMPRESSION = SNAPPY;
...
FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/customer
(FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET');
...
CREATE EXTERNAL TABLE BLAZEJ_DB.STAGE.TB_CUSTOMERS_EXT_TAB 
LOCATION = @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/customer
FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET;

SELECT * FROM BLAZEJ_DB.STAGE.TB_CUSTOMERS_EXT_TAB LIMIT 10;
...
CREATE OR REPLACE EXTERNAL TABLE BLAZEJ_DB.STAGE.TB_CUSTOMERS_EXT_TAB (
...
SELECT * EXCLUDE VALUE FROM BLAZEJ_DB.STAGE.TB_CUSTOMERS_EXT_TAB LIMIT 10;


--Załadowanie danych z pliku bezpośrednio do tabeli przy pomocy COPY INTO

CREATE TABLE IF NOT EXISTS BLAZEJ_DB.STAGE.TB_CUSTOMERS (
    BIRTHDAY_DATE DATE,
    CHILDREN_COUNT TEXT,
    CITY TEXT,
    COUNTRY TEXT,
...
SELECT * FROM BLAZEJ_DB.STAGE.TB_CUSTOMERS LIMIT 10;

-- Zaladowanie danych z pliku do tabeli
-- https://docs.snowflake.com/en/sql-reference/sql/copy-into-table
COPY INTO BLAZEJ_DB.STAGE.TB_CUSTOMERS FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/customer
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
...
SELECT * FROM BLAZEJ_DB.STAGE.TB_CUSTOMERS LIMIT 10;
SELECT COUNT(*) FROM BLAZEJ_DB.STAGE.TB_CUSTOMERS;
...
TRUNCATE TABLE BLAZEJ_DB.STAGE.TB_CUSTOMERS;
COPY INTO BLAZEJ_DB.STAGE.TB_CUSTOMERS FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/customer
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
...
FROM TABLE(
    INFER_SCHEMA(
        LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/truck',
        FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET'
    )
);
...
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_TRUCKS
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
        FROM TABLE(
            INFER_SCHEMA(
...
DESC TABLE BLAZEJ_DB.STAGE.TB_TRUCKS;

COPY INTO BLAZEJ_DB.STAGE.TB_TRUCKS FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/truck
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
SELECT COUNT(*) FROM BLAZEJ_DB.STAGE.TB_TRUCKS;

SELECT * FROM BLAZEJ_DB.STAGE.TB_TRUCKS LIMIT 10;

--Utworeznie reszty tabeli dla danych FrostByte 
--Country
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_COUNTRY
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
        FROM TABLE(
            INFER_SCHEMA(
                LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/country',
                FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET'
            )
        )
    );

COPY INTO BLAZEJ_DB.STAGE.TB_COUNTRY FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/country
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;    

--Location
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_LOCATION
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
        FROM TABLE(
            INFER_SCHEMA(
                LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/location',
                FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET'
            )
        )
    );

COPY INTO BLAZEJ_DB.STAGE.TB_LOCATION FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/location
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;

--Menu
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_MENU
USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
    FROM TABLE(
        INFER_SCHEMA(
            LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/menu',
            FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET'
        )
    )
);

COPY INTO BLAZEJ_DB.STAGE.TB_MENU FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/menu
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;    

--Franchise
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_FRANCHISE
USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
    FROM TABLE(
        INFER_SCHEMA(
            LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/franchise',
            FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET'
        )
    )
);

COPY INTO BLAZEJ_DB.STAGE.TB_FRANCHISE FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/franchise
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;    
...
SELECT *
    FROM TABLE(
        INFER_SCHEMA(
            LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/order_header',
            FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET'
        )
    );
...
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_ORDER_HEADER
USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
    FROM TABLE(
        INFER_SCHEMA(
...
COPY INTO BLAZEJ_DB.STAGE.TB_ORDER_HEADER FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/order_header
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;

--Order detail

CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.TB_ORDER_DETAIL
USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
    FROM TABLE(
        INFER_SCHEMA(
            LOCATION => '@BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/order_detail',
            FILE_FORMAT => 'BLAZEJ_DB.STAGE.SNAPPY_PARQUET',
            MAX_FILE_COUNT => 5
        )
    )
...
COPY INTO BLAZEJ_DB.STAGE.TB_ORDER_DETAIL FROM @BLAZEJ_DB.STAGE.TASTYBYTES_RAW_STAGE/pos/order_detail
    FILE_FORMAT = BLAZEJ_DB.STAGE.SNAPPY_PARQUET
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;


-- Kolumna w TB_MENU oryginalnie jest typu OBJECT, INFER_SCHEMA okreslil go jako TEXT/VARCHAR/STRING
SELECT * FROM BLAZEJ_DB.STAGE.TB_MENU LIMIT 10;
SELECT MENU_ITEM_HEALTH_METRICS_OBJ FROM BLAZEJ_DB.STAGE.TB_MENU LIMIT 10;
DESC TABLE BLAZEJ_DB.STAGE.TB_MENU;

SELECT PARSE_JSON(MENU_ITEM_HEALTH_METRICS_OBJ) FROM BLAZEJ_DB.STAGE.TB_MENU LIMIT 10;
--Bezpieczna wersja to TRY_PARSE_JSON

-- Przyklad eksplodowania JSON za pomocą funkcji FLATTEN
CREATE OR REPLACE TABLE BLAZEJ_DB.STAGE.HEALTH_METRIX_EXPLODED 
AS
SELECT SEQ, KEY, PATH, INDEX, VALUE, THIS 
FROM BLAZEJ_DB.STAGE.TB_MENU menu,
TABLE(FLATTEN(
    INPUT => PARSE_JSON(menu.MENU_ITEM_HEALTH_METRICS_OBJ)
));

SELECT * FROM BLAZEJ_DB.STAGE.HEALTH_METRIX_EXPLODED LIMIT 10;

-- Przykład lineage'u

CREATE SCHEMA BLAZEJ_DB.TRANSFORMED;
CREATE OR REPLACE TABLE BLAZEJ_DB.TRANSFORMED.TB_CUSTOMER_AGE (CUSTOMER_ID NUMBER, POSTAL_CODE TEXT, AGE NUMBER)
AS
SELECT CUSTOMER_ID, POSTAL_CODE, DATEDIFF('year', CURRENT_DATE(), BIRTHDAY_DATE) AS AGE
FROM BLAZEJ_DB.STAGE.TB_CUSTOMERS;

CREATE OR REPLACE TABLE BLAZEJ_DB.TRANSFORMED.TB_AVG_AGE_PER_CODE ( POSTAL_CODE TEXT, AVG_AGE NUMBER)
AS
SELECT cust.POSTAL_CODE, AVG(AGE) AS AVG_AGE FROM BLAZEJ_DB.TRANSFORMED.TB_CUSTOMER_AGE cust_age
INNER JOIN BLAZEJ_DB.STAGE.TB_CUSTOMERS cust ON cust_age.customer_id = cust.customer_id
GROUP BY cust.POSTAL_CODE;


/*
    W tej czesci pokaze troche interakcji z chmura AWS:
    - Jak dziala Stage z prawdzimym S3 Storage
    - Czym jest Snowpipe
